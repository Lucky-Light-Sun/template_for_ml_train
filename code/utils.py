...
def plot_to_tensorboard(
    cur_stage, writer, loss_critic, loss_gen, real, fake, tensorboard_step
):
    ...
    with torch.no_grad():
        ...
        # 多加入一下存储图片的操作
        if not os.path.exists(os.path.join(config.IMAGE_SAVE_DIR, f'{cur_stage}')):
            os.makedirs(os.path.join(config.IMAGE_SAVE_DIR, f'{cur_stage}'))

        file_path = os.path.join(config.IMAGE_SAVE_DIR, f'{cur_stage}/{tensorboard_step:05d}.png')
        if real.shape[2] <= 20:
            # 8
            nrow = 8
        elif real.shape[2] <= 80:
            # 4
            nrow = 4
        elif real.shape[2] <= 160:
            # 2
            nrow = 2
        else:
            # 1
            nrow = 1
        save_image(
            torch.cat([img_grid_fake, img_grid_real], dim=1), file_path,
